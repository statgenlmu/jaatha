...
calcStatLLH.Stat_PoiInd <- function(sum_stat, glm_fitted, param, scaling_factor = 1) {
  loglambda <- sapply(glm_fitted, predict, newdata=data.frame(t(as.matrix(param))))
  
  #if glm did not converge, take sum(SS[s]) or a small number like 0.5 
  loglambda[!sapply(glm_fitted[[name]], function(x) x$converged)] <- 0.5 
