calcFpcBreaks <- function(dm, seg.sites, props=c(.2, .5), group=0) {
  ...
  if (group == 0) {
    ...
    dm@options[['fpc.breaks.between']] <- calcBreaks(fpc.percent[, 6], props)
  } else {
    ...
    dm@options[[group.name]][['fpc.breaks.between']] <- calcBreaks(fpc.percent[, 6], props)
  }
