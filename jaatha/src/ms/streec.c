...
//WCC	char c, event ;
	char event = '\0';
//WCC	int re(), xover(), cinr(), cleftr(), eflag, cpop, ic  ;
	int eflag, cpop = 0, ic;
//WCC	int nsam, npop, nsites, nintn, *inconfig ;
	int nsam, npop, nsites, *inconfig ;
//WCC	double r,  f, rf,  track_len, *nrec, *npast, *tpast, **migm ;
...
//WCC	int  el, lsg, lsgm1,  ic,  is, in, spot;
...
{
	struct seg *pseg ;
//WCC	int   lsg, lsgm1,  ic,  is, in, spot;
	int   ic,  is;
//WCC	double ran1(), x, sum, len  ;
	double x, sum, len  ;
...
//WCC	struct seg *pseg ;
	struct seg *pseg = NULL;
//WCC	int len,  el, lsg, lsgm1,  ic,  is, in, spot, endic ;
...
//WCC	int i,  lsg, lsgm1, newsg,  jseg, k,  in, spot;
