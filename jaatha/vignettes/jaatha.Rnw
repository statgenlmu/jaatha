...
-- with the following restriction: On Windows the parallelization and finite sites
simulation using seq-gen is currently not working.

A more detailed description of the algorithm can be found in
\cite{mathew_composite_2012}. 
...
if(!file.exists('cache/saved_objects')) dir.create('cache/saved_objects')
...
\noindent
Hence we perform two independent searches, starting in the \verb@best.start.pos@
best starting positions we choose before 
and according to the general options we choose during initialization, which are stored in the 
\verb@jaatha@ object. In each step, we build a block with \verb@half.block.size@ in each direction
(on a logarithmic scale) and perform simulations for \verb@sim@ random parameter combinations within
...
As described in \cite{mathew_composite_2012}, you can use finite sites mutation 
...
installed on your system. On Debian/Ubuntu can install it running 
