...
data from multiple individuals from both species and -- optinally but
...
\cite{naduvilezhath_jaatha:_2011}. Please cite this paper when using Jaatha in a
publication.
Further explanation of all functions used here can be found in the R help pages of Jaatha
via \verb@?function@.  The package itself can be obtained from CRAN
using 
<<install, run=FALSE>>=
