...
A detailed (but a bit a outdated) description of Jaatha can be found in 
\cite{naduvilezhath_jaatha_2011}, which also is the correct citation for Jaatha. 
Further explanation of all functions used can be found in the R help pages of Jaatha
as well as in the Jaatha manual.
...
$0.01$ to $5$ and $1$ to $20$ -- doing simulations for are small part of the parameter space
around the current position (to which we refer as a \emph{block}). It then searches the new maximum
...
As default, it calculates the Joint Site Frequency Spectrum (JSFS) of the data and further summaries
this by taking different sums over the JSFS. Please refer to \cite{tellier_estimating_2011} for a detailed
description.
...
\noindent
For more availbe options, please refer to \Verb@?Jaatha.initialize@ or the Jaatha manual.
...
\noindent
Important: Please note that the parameter values here will lead to a fast, but a bit 
inaccurate search. For a typical usage, we recommend setting \verb@nSim=400@, 
\verb@nFinalSim=200@ and \verb@epsilon=0.2@.
...
The algorithm stop when the score has not changed more than \verb@epsilon@ for five consecutive 
steps or step \verb@nMaxStep@ is reached. To avoid getting stuck in local maxima, 
the \verb@weight@ option decreases the weigh of simulations of previous blocks.
