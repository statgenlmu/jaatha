...
Hence, we could propose the simple demographic model descriped in Figure~\ref{fig:dm}.
...
\end{center}
\caption{
A simple demographic model: The ancestral population spilts into two populations $\tau$ time 
