...
@
\noindent
Here, there is a big reduction in the scores after the first three blocks. 
This is suggesting that we either use the first three blocks as starting 
...
confidence intervals for the estimated parameters. As usual, bootstrapping
...
core). We usually use $32$ cores on powerful computation servers for this.  
