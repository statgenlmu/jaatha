...
test_that("test.fitGlm", {
    ...
    expect_equal(length(sim.data.csi[[1]]$poisson.vector), length(glms.fitted.csi$poisson.vector))
    expect_true(all(sapply(glms.fitted.csi$poisson.vector, is)[1, 
        ] == "glm"))
    glms.fitted.tt <- fitGlm(sim.data.tt, jaatha.tt)
    expect_true(is.list(glms.fitted.tt))
    expect_equal(1, length(glms.fitted.tt))
    expect_true(is.list(glms.fitted.tt$jsfs))
    expect_equal(length(jaatha.tt@sum.stats$jsfs$value.transformed), 
        length(glms.fitted.tt$jsfs))
    expect_true(all(sapply(glms.fitted.tt$jsfs, is)[1, ] == "glm"))
