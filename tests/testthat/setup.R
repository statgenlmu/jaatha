...
sim.data.tt  <- jaatha:::simulateWithinBlock(10, block.test, jaatha.tt)
...
# Finite Sites Models
if (jaatha:::checkForSeqgen(FALSE)) {
  test_seqgen <- TRUE
  ...
  warning("No seqgen executable found. Skipping tests.")
  test_seqgen <- FALSE
}

if (jaatha:::checkForMsms(FALSE)) {
  test_msms <- TRUE
  # Selection Model
  dm.sel <- jaatha:::dm.addPositiveSelection(dm.mig, 1000, 2000, population=2,
                                             at.time='tau/2')
  dm.sel <- dm.setLociNumber(dm.sel, 3)
  dm.sel <- jaatha:::calcFpcBreaks(dm.sel, seg.sites)
} else {
