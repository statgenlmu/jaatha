...
test_that("Jaatha.initialization.fpc_groups", {
    ...
    expect_true(sum(jaatha.fpc@sum.stats[["fpc.1"]]$value) > 
        0)
    expect_true(sum(jaatha.fpc@sum.stats[["fpc.2"]]$value) > 
        0)
    expect_true(sum(jaatha.fpc@sum.stats[["fpc.3"]]$value) > 
        0)
})
...
test_that("init", {
    ...
    sum.stat2 <- list(method = "poisson.transformed", value = matrix(1, 
        3, 3), transformation = diag)
    ...
    expect_true(all(jaatha@sum.stats$ss2$value.transformed == 
        c(1, 1, 1)))
